...
const getScriptUrl = function(embededPage) {
	...
	return 'https://www.youtube.com' + match[0].groups.path + '/base.js';
};
...
const getDecipherCode = function(script) {
	/*
		source from youtube-dl: _parse_sig_js(self, jscode);
	*/
	const commonPtrn = [
	...
	if (!entryFunc) throw new Error('Cannot find entry function for decryption');
	const entryFuncCode = getVarDefinition(script, entryFunc);
	printLog(`varName: ${entryFunc}, def: ${entryFuncCode}`);
	const funcList = [entryFuncCode];
	let maxAllowedTry = 100;
	return (function () {
		...
		do {
			try {
				...
				if (e.name === 'ReferenceError') {
					const fname = getUndefinedVarName(e.message);
					const fcode = getVarDefinition(script, fname);
					printLog(`varName: ${fname}, def: ${fcode}`);
					eval(fcode);
					funcList.push(fcode);
				} else {
...
const getFuncDef = function(script, funcName) {
	// this function does vague search for base.js functions, which is vulnerable towards string literals,
	// do not use it for general function search
	const match = script.match(new RegExp(`${funcName}\\s*=\\s*function\\([^)]*\\)\\s*{`, 's'));
	if (!match) throw new Error(`Function ${funcName} not found`);
	let mLength = match[0].length;
	const sIndex = match.index + mLength;
	let index = sIndex;
	let stackCount = 1;
	while (index < script.length) {
		const c = script[index];
		++index;
		if (c === '{')  {
			++stackCount;
		} else if (c === '}') {
	...
	if (stackCount > 0) throw new Error(`Unbalanced brackets in function ${funcName}`);
	return match[0]+script.substring(sIndex, index);
}

const getVarDeclarator = function(script, varName) {
	const match = script.match(new RegExp(`^\\s*(?<dec>var|let|const)\\s+?(?:\\w+\\s*,\\s*)*?'${varName}\\b')`));
	if (!match) throw new Error(`No such variable ${varName}`);
	return match.groups.dec;
};

const getUndefinedVarName = function(errMsg) {
	printLog(`getUndefinedVarName: ${errMsg}`)
	const match = errMsg.match(/^(?<varName>\w+) is not defined$/);
	if (!match) throw new Error(`Unexpected error: ${errMsg}`);
	else printLog(match.groups.varName);
...
const getVarDefinition = function(script, varName) {
	// accept var definition in following style:
	// (var|let|const|) varName=(*|function(*))
	// special checking: varName cannot follow any character other than [,;\s]
	// assume var definition ends with ';' and no string literals included
	...
	if (!dec.groups.f) {
		...
		let c = script[sIdx];
		if (c === '{' || c === '[' || c === '(') {
			eIdx = balanceSymbol(script, sIdx);
		} else {
			eIdx = sIdx;
			do {
				c = script[++eIdx];
...
const escapeRegexChar = function(s) {
	return string.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
};

const balanceSymbol = function(script, startIdx) {
	// find end index of symbol that balances code starts with start index
	// assume no string literals in code
	let idx = startIdx;
	let stack = [];
...
module.exports = { getScriptUrl, getDecipherCode, Searcher, Decrypter };
